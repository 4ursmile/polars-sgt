...
fn idx_dtype(input_fields: &[Field]) -> PolarsResult<Field> {
    let field = Field::new(
...
#[polars_expr(output_type_func=idx_dtype)]
...
fn sgt_transform_output(_input_fields: &[Field]) -> PolarsResult<Field> {
    let fields = vec![
...
#[polars_expr(output_type_func=sgt_transform_output)]
fn sgt_transform(inputs: &[Series], kwargs: SgtTransformKwargs) -> PolarsResult<Series> {
    impl_sgt_transform(
        inputs,
        kwargs.kappa,
        kwargs.length_sensitive,
        &kwargs.mode,
        &kwargs.time_penalty,
        kwargs.alpha,
        kwargs.beta,
